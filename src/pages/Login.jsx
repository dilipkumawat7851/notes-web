    ...
    <div className="min-h-screen flex items-center justify-center bg-[#111111] px-4">
      <div className="w-full max-w-md bg-[#1A1A1A] border border-[#2D2D2D] rounded-lg shadow-xl p-10 text-white">
        ...
        <div className="text-center mb-10">
          <h1 className="text-4xl font-serif mb-3">Log In</h1>
          <p className="text-sm text-slate-400">
            Need an account? <Link to="/register" className="text-[#84C4ED] hover:underline">Sign Up</Link>
          </p>
        </div>

        {error && (
          <div className="mb-6 p-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded text-sm text-center">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end">
             <Link to="/forgot-password" className="text-xs text-slate-400 hover:text-[#84C4ED]">Forgot password?</Link>
          </div>
